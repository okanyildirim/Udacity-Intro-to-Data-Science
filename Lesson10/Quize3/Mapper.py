...
def mapper():
    ...
    for line in sys.stdin:
        ...
        if first_line:
            ...
            DATEn_index = data.index("DATEn")
            TIMEn_index = data.index("TIMEn")
            logging.info(UNIT_index)
            logging.info(ENTRIESn_hourly_index)
            logging.info(DATEn_index)
            logging.info(TIMEn_index)
            first_line = False
        else:
            print("{0}\t{1}\t{2}\t{3}".format(data[UNIT_index], data[ENTRIESn_hourly_index],data[DATEn_index],data[TIMEn_index]))
